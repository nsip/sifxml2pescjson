...
to make SIF and PESC data available in JSON as a first-class data format. SIF to date supports JSON for its data exchanges via the 
...
both XML and JSON for the foreseeable future.) However, in seeking to preserve the structure of the source XML, the Goessner notation introduces a number of undesirable characteristics into the JSON it generates:
...
The https://www.xml.com/pub/a/2006/05/31/converting-between-xml-and-json.html[PESC JSON conversion] addresses all these issues.
...
Each SIF schema instance therefore has its own associated transform scripts. The code in this repository generates those transform scripts for a SIF schema instance.
...
Currently a local instance of `scripts/x2js.js` is used: 
https://github.com/x2js/x2js/pull/76[a bug has been identified] with the current release of `x2js`, which has not yet been incorporated into a new release.
